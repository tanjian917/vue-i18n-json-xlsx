...
function savexlsx2Js(data,name='xxx.json') {
    console.log('正在保存json文件:' + path.join(config.lpath,name));
...
function json2Xlsx() {
    console.log('正在扫描'+config.lpath+'目录下的json文件.');
    let files = fs.readdirSync(config.lpath),langsSet = new Set();
    files.forEach(filename=>{
        let filePath = path.join(config.lpath,filename);
        let stats = fs.statSync(filePath);
        if(stats.isFile() && filename.charAt(0) != '.' && path.extname(filePath).toLowerCase() === ".json"){
            langsSet.add({
    ...
    if(stat.isFile()){
        ...
        headers.forEach((val,index)=>{
            if(val.slice(0,2) == 'c-')valueOffset ++;
                ...
                savexlsx2Js(interatorObj2Str(result,config.format),val+'.json');
            }
        });
    }
}
...
function bootstrap() {
    ...
    if(config.hasOwnProperty('help')){
        console.log(`vue-i18n-json-xlsx 参数说明:
